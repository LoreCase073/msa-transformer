...
import os
from model import MSATransf, MSATransfNoCol
...
from accuracy_images import accuracy_images
...
if __name__== '__main__':
    ...
    msat = MSATransf(args, alphabet_size, padding_idx,
                    mask_idx)
    #msat = MSATransfNoCol(args, alphabet_size, padding_idx, mask_idx)
    ...
    with torch.no_grad():
        with tqdm(test_loader, unit='batch') as vepoch:
            for msa, _, _, distances,name in vepoch:
                ...
                # vacc = accuracy_images(contacts, labels, name)
                csv_list.append((name, str(vacc['LR_L']), str(vacc['LR_L/5'])))
                lrl5 += vacc['LR_L/5']
                lrl2 += vacc['LR_L/2']
                lrl += vacc['LR_L']
            time.sleep(0.1)
    ...
    with open('../Dataset/test_set/MSAT_precision.csv', 'w') as ts:
